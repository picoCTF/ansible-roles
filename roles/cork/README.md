...
| cork_base_pins | `CORK_BASE_PINS`, the base image digest pin file. Unset by default, so cork derives `<CORK_DIR>/.base-pins.json` and the pins follow the tree — the file is **committed to the challenge repository** and versioned with the Dockerfiles it pins, and `cork-build --dir` relocates the tree for a CI runner building from its workspace. An absolute path here would keep resolving to the provisioned copy after the tree moved, which refingerprints and rebuilds the corpus rather than failing. Nothing creates the file; `cork-build pins` writes it. Omitted on an external daemon, which answers 409 to the pin endpoints. | unset (`<cork_dir>/.base-pins.json`) |
