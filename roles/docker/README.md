...
| `docker_bip` | Address of the default bridge (`docker0`), written as daemon.json's `bip`. `null` leaves it out, so `docker0` is carved out of `network_ip_pools` like every other network -- right on a worker, where nothing attaches to the default bridge at all (cork gives every instance a network of its own, sized by `network_prefix_length`). A host that **builds** needs its own block: BuildKit puts each `RUN` step's networking on the default bridge, so a /29 there fails a parallel build with `no available IPv4 addresses on this network's address pools: bridge`. `docker_builder` sets it (10.200.0.1/24). Overlapping `network_ip_pools` is waste rather than breakage -- measured on Docker 29.8, the allocator skips the bridge's subnet and dockerd starts either way -- until the pools run out, at which point `docker network create` fails and every launch with it. | `null` |
