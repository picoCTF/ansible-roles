...
| `docker_reaper_images_command` | Image sweep arguments, run as a second `ExecStart` after the container sweep. Requires docker-reaper ≥ v1.2.0, which introduced the `images` subcommand. `--disk-path` names the data volume (`/var/lib/docker` with `storage_quotas` off) so the reaper does not ask dockerd for it with a `/info` call every run. | `images --threshold 89 --target 84 --disk-path /mnt/docker-data` |
